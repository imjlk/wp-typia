...
describe('package version helpers', () => {
	test('prefers the workspace create package manifest over installed package manifests', async () => {
		...
		expect(module.getPackageVersions()).toEqual({
			...
			ttscLintPackageVersion: '0.30.4',
			ttscLintPluginWpPackageVersion: '^0.6.1',
			ttscLintPluginWpTtscPeerRange: '>=0.23.0 <0.31.0',
			ttscPackageVersion: '^0.30.4',
			tsxPackageVersion: '^0.30.4',
			typiaPackageVersion: '^14.0.6',
			ttscUnpluginPackageVersion: '^0.30.4',
			typiaUnpluginPackageVersion: '^0.30.4',
	...
	test('keeps the TS7 toolchain baseline when an older generated project is installed', async () => {
		...
		expect(versions.ttscLintPackageVersion).toBe('0.30.4');
		expect(versions.ttscPackageVersion).toBe('^0.30.4');
		expect(versions.ttscUnpluginPackageVersion).toBe('^0.30.4');
		expect(versions.typescriptPackageVersion).toBe('^7.0.2');
		expect(versions.typiaPackageVersion).toBe('^14.0.6');
	});
