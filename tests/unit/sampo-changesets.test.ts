...
function createTempRepo() {
  const repoRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'wp-typia-sampo-test-'),
  );
  ...
  fs.mkdirSync(path.join(repoRoot, 'examples', 'compound-patterns'), {
    recursive: true,
  });
  fs.mkdirSync(path.join(repoRoot, 'examples', 'private-example'), {
    recursive: true,
  });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { name: '@wp-typia/project-tools', version: '0.1.0' },
      null,
      2,
    ),
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { name: 'private-example', private: true, version: '0.1.0' },
      null,
      2,
    ),
  );
