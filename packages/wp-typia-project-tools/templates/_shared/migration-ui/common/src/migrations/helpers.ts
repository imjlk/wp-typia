  ...
  switch (attribute.ts.kind) {
    ...
    case 'array': {
      ...
      return value.every((item) =>
        manifestMatchesAttribute(attribute.ts.items as ManifestAttribute, item),
      );
    }
    case 'object': {
      if (!isPlainObject(value)) {
      ...
      for (const [key, property] of Object.entries(
        ...
        if (!manifestMatchesAttribute(property, value[key])) {
          return false;
        }
      }
      return true;
...
function matchesUnionAttribute(
  ...
  return manifestMatchesAttribute(branches[branchKey], value);
}
