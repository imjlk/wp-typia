...
function isSectionWrapperCandidate(
  ...
  return getClassNameTokens(block.attributes).includes(
    convention.baseClassName,
  );
}
...
export function extractPatternSectionRolesFromAttributes(
  ...
  return extractPatternSectionRolesFromNormalizedAttributes(
    attributes,
    normalized,
  );
}
...
export function validatePatternContentSectionRoles({
  ...
  for (const invalidRole of uniqueValues(
    invalidRoles.map(({ role }) => role),
  )) {
