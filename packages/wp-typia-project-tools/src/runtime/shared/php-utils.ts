...
export function hasPhpFunctionDefinition(
  ...
  return new RegExp(`function\\s+${escapeRegex(functionName)}\\s*\\(`, 'u').test(
    source,
  );
}
...
function findPhpLineBoundary(
  ...
  return {
    contentEnd: source[newlineIndex - 1] === '\r'
      ? newlineIndex - 1
      : newlineIndex,
...
function getPhpOpenTagLength(source: string, index: number): number {
  ...
  if (source.slice(index, index + 5).toLowerCase() === '<?php') {
    ...
    return nextCharacter === undefined || isPhpWhitespace(nextCharacter)
      ? 5
      : 0;
  }
  return 0;
