...
export const PACKAGE_MANAGER_IDS = PACKAGE_MANAGER_DATA.map(
  (manager) => manager.id,
) as PackageManagerId[];
...
function replaceBunRunCommands(content: string, packageManagerId: PackageManagerId): string {
  ...
  while (cursor < content.length) {
    ...
    const scriptNameMatch = /^[A-Za-z0-9:_-]+/.exec(
      content.slice(scriptNameStart),
    );
    if (!scriptNameMatch) {
      result += marker;
    ...
    const { args, cursor: nextCursor } = consumeCommandArguments(
      content,
      argsStart,
    );
