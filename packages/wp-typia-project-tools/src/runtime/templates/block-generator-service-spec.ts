...
} from '../add/alternate-render-targets.js';
import {
...
export function buildTemplateVariablesFromBlockSpec(spec: BlockSpec): ScaffoldTemplateVariables {
  ...
  const hasAlternateEmailRenderTarget = alternateRenderTargets.includes(
    'email',
  );
  ...
  const dataStorageMode = persistenceEnabled
    ? spec.persistence.dataStorageMode
    : 'custom-table';
