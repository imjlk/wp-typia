...
function isBlockedPath(pathLabel: string, blockedPaths: readonly string[]): boolean {
  return blockedPaths.some(
    (blockedPath) => pathLabel === blockedPath || pathLabel.startsWith(`${blockedPath}.`),
  );
}
...
export function createMigrationFuzzPlan(
  ...
  const currentLeafDescriptors = flattenManifestLeafAttributes(
