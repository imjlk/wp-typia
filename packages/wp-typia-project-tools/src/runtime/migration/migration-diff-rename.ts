...
export function createRenameCandidates({
  ...
  const removedLeafDescriptors = oldLeafAttributes.filter(
    (descriptor) => !newLeafMap.has(descriptor.currentPath),
  );
  const addedLeafDescriptors = newLeafAttributes.filter(
    (descriptor) => !oldLeafMap.has(descriptor.currentPath),
  );
...
function assessRenameCandidate(
  ...
  if (!isRenameCandidateShapeCompatible(
    oldAttribute,
    newAttribute,
    isUnionRenameCompatible,
  )) {
  ...
  const score =
		getParentPath(legacyPath) === getParentPath(currentPath)
...
function scoreRenameSimilarity(legacyPath: string, currentPath: string): number {
  ...
  const jaccard = overlap.length / new Set([
    ...legacyTokens,
    ...currentTokens,
  ]).size;
