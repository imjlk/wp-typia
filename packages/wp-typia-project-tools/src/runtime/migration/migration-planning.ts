...
export function hasSnapshotForVersion(
  ...
  return fs.existsSync(
    getSnapshotManifestPath(state.projectDir, block, version),
  );
}
...
export function getSelectedEntriesByBlock(
  ...
  if (missingEntries.length > 0) {
    const missingLabels = missingEntries
			...
			.join(', ');
    const missingVersions = [...new Set(missingEntries.map(({ version }) => version))].sort(
  ...
  return groupEntriesByBlock(
    discoveredEntries.filter((entry) =>
      targetVersions.includes(entry.fromVersion),
    ),
  );
}
...
function groupEntriesByBlock(entries: ReturnType<typeof discoverMigrationEntries>): Record<string, typeof entries> {
  return entries.reduce<Record<string, typeof entries>>(
    (accumulator, entry) => {
