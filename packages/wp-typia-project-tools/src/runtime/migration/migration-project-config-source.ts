...
export function hasLegacyConfigKeys(source: string): boolean {
  ...
  return /\bcurrentVersion\s*:/u.test(
    topLevelView,
  ) || /\bsupportedVersions\s*:/u.test(topLevelView);
}
...
export function parseMigrationConfig(source: string): MigrationConfig {
  ...
  const currentMigrationVersion = matchRootConfigValue(
    source,
    'currentMigrationVersion',
  );
  const snapshotDir = matchRootConfigValue(source, 'snapshotDir');
  const supportedMigrationVersions = matchRootConfigStringArrayValue(
    source,
  ...
  if (!currentMigrationVersion || !snapshotDir || !supportedMigrationVersions) {
    throw new Error(
      'Unable to parse migration config. Regenerate with `wp-typia migrate init --current-migration-version v1`.',
    );
  }
  ...
  if (!isMigrationVersionLabel(currentMigrationVersion)) {
    ...
    throw new Error(
      `Invalid current migration version: ${currentMigrationVersion}. Expected vN with N >= 1.`,
    );
  }
  ...
  for (const version of supportedMigrationVersions) {
    if (!isMigrationVersionLabel(version)) {
      ...
      throw new Error(
        `Invalid supported migration version: ${version}. Expected vN with N >= 1.`,
      );
    }
  }
