...
import { getLocalTtsxBinary, readJson } from './migration-utils.js';
...
export function verifyProjectMigrations(
  ...
  const blockEntries = getSelectedEntriesByBlock(
    state,
    targetVersions,
    'verify',
  );
  ...
  for (const version of snapshotVersions) {
    for (const block of state.blocks) {
      ...
      const snapshotIsOptional =
        !hasSnapshot && isSnapshotOptionalForBlockVersion(
          state,
          block,
          version,
        );
      ...
      recordCheck(
        ...
        legacySingleBlock
          ? `Snapshot ${version}`
          : `Snapshot ${block.blockName} @ ${version}`,
        hasSnapshot
  ...
  try {
    ...
    for (const [filePath, expectedSource] of expectedGeneratedFiles) {
      const inSync =
        ...
        fs.readFileSync(filePath, 'utf8') === expectedSource;
      recordCheck(
        inSync ? 'pass' : 'fail',
  ...
  for (const version of targetVersions) {
    for (const block of state.blocks) {
      ...
      recordCheck(
        ...
        legacySingleBlock
          ? `Fixture ${version}`
          : `Fixture ${block.blockName} @ ${version}`,
