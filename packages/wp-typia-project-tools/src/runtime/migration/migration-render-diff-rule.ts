...
export function renderMigrationRuleFile({
  ...
  for (const candidate of activeRenameCandidates) {
    lines.push(
      `\t${renderObjectKey(candidate.currentPath)}: "${escapeForCode(candidate.legacyPath)}",`,
    );
  }
  ...
  lines.push('');
  lines.push(
    `export function migrate(input: Record<string, unknown>): ${currentTypeName} {`,
  );
  lines.push(`\treturn {`);
  ...
  for (const key of Object.keys(currentAttributes)) {
    ...
    for (const suggestion of diff.summary.transformSuggestions.filter(
      ...
      lines.push(
        `\t\t// ${MIGRATION_TODO_PREFIX} review transforms[${JSON.stringify(suggestion.currentPath)}]`,
      );
    }
    lines.push(
