...
export function createTransformSuggestions({
  ...
  const activeRenameTargets = new Set(
    renameCandidates.filter((candidate) => candidate.autoApply).map(
      (candidate) => candidate.currentPath,
    ),
  );
  const oldLeafMap = new Map(
    oldLeafAttributes.map(
      (descriptor) => [descriptor.currentPath, descriptor] as const,
    ),
  );
  const newLeafMap = new Map(
    newLeafAttributes.map(
      (descriptor) => [descriptor.currentPath, descriptor] as const,
    ),
  );
  ...
  for (const currentPath of [
    ...
    const bestRenameCandidate = renameCandidates.find(
      (candidate) => candidate.currentPath === currentPath,
    );
    if (bestRenameCandidate && !bestRenameCandidate.autoApply) {
      suggestions.push({
        bodyLines: buildTransformBodyLines(
          currentAttribute,
          bestRenameCandidate.legacyPath,
        ),
        attribute: currentAttribute,
        currentPath,
...
export function describeConstraintChange(
  ...
  if (newAttribute.wp.enum && JSON.stringify(
    newAttribute.wp.enum,
  ) !== JSON.stringify(oldAttribute.wp.enum)) {
