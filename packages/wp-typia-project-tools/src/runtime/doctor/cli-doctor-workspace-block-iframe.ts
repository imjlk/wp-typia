...
function isWorkspaceBlockSaveSource(relativePath: string): boolean {
  const normalizedBaseName = path.basename(
    relativePath,
...
function findWorkspaceBlockGlobalDomAccesses(
  ...
  return editorSources.flatMap(({ relativePath, source }) => {
    ...
    const matches = maskedSource.matchAll(
      WORKSPACE_BLOCK_IFRAME_GLOBAL_DOM_PATTERN,
    );
    ...
    for (const match of matches) {
      ...
      findings.push(
        `${relativePath}:${getSourceLineNumber(source, matchIndex)}`,
      );
    }
...
export function getWorkspaceBlockIframeCompatibilityChecks(
  ...
  const metadataResult = readWorkspaceBlockIframeMetadata(
    projectDir,
    blockSlug,
  );
  ...
  const editorSources = collectWorkspaceBlockEditorSources(
    projectDir,
    blockSlug,
  );
  ...
  const blockWrapperStatus: DoctorCheck['status'] =
		editorWrapperSources.length === 0 || hasEditorBlockPropsUsage
      ? 'pass'
      : 'warn';
  ...
  return [
    ...
    createDoctorCheck(
      ...
      localStyleFiles.length === 0 || hasRegisteredEditorStyles
        ? 'pass'
        : 'warn',
